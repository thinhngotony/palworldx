...
class DashboardHTTPTests(unittest.TestCase):
    ...
    def test_authenticated_mod_list_is_json_safe(self):
        cookie = self.login()
        fake_mod = {"id": "sample", "name": "Sample", "metadata": {"version": 1}}
        with mock.patch.object(dashboard, "mod_manager", object()), mock.patch.object(dashboard, "_mod_manager_call", return_value=[fake_mod]):
    ...
    def test_control_mutation_requires_single_use_confirmation(self):
        ...
        with mock.patch.object(dashboard, "is_server_running", return_value=False):
            status, _, body = self.request("POST", "/api/control", {"action": "stop", "maintenance_confirmation": token}, cookie)
        self.assertEqual(status, 200)
    ...
    def test_mod_mutation_requires_confirmation_then_calls_backend(self):
        ...
        self.assertEqual(status, 409)
        token = json.loads(body)["confirmation_token"]
        with mock.patch.object(dashboard, "mod_manager", backend), mock.patch.object(dashboard, "PALWORLD_DIR", "/tmp/test-dashboard-no-production"):
            status, _, body = self.request("POST", "/api/mods/sample/enable", {"maintenance_confirmation": token}, cookie)
        self.assertEqual(status, 200)
    ...
    def test_expired_confirmation_is_rejected(self):
        ...
        with mock.patch.object(dashboard, "MAINTENANCE_CONFIRMATION_TIMEOUT", 1), mock.patch.object(dashboard, "is_server_running", return_value=False):
            status, _, body = self.request("POST", "/api/control", {"action": "restart", "maintenance_confirmation": "expired"}, cookie)
        self.assertEqual(status, 409)
